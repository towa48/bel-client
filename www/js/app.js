(function(angular, undefined) {
   ...
   * Error modal dialog
   */
  var ErrorModal = function($ionicModal, scope, message) {
    this.$scope = scope.$new(true);
   ...
   * ANGULAR APPLICATION
   */
  angular.module('BelApp', ['BelApp.Config', 'ionic'])
   ...
   * BelClient Service
   */
  .service('BelClient', ['$http', '$q', 'apiUrl', function($http, $q, apiUrl) {
    ...
    $scope.send = function(e) {
      belClient.send($scope.form)
        .done(function(data, status, headers) {
          ...
          new ErrorModal($ionicModal, $scope, data.errorText).show(function() {
            $scope.goHome();
          });
        })
